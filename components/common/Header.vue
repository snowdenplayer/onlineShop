    ...
    <header class="header main-header">
      <nav class="nav">
        <div class="nav__item-first">
          ...
          <div class="logo_items">
            <ul class="items-wrapper">
              <nuxt-link to="/" tag="li" :class="{active: activeTab === 'Home'}">Головна</nuxt-link>
              <nuxt-link to="/about" tag="li" :class="{active: activeTab === 'About'}">Про нас</nuxt-link>
              <nuxt-link to="/categories?mode=main_categories" tag="li"
                         :class="{active: activeTab === 'Goods' || activeTab === 'Categories'}">Товари
              </nuxt-link>
              <nuxt-link to="/contacts" tag="li" class="last" :class="{active: activeTab === 'Contacts'}">Контакти
              </nuxt-link>
            </ul>
          </div>
        </div>
        <div class="nav__item-second">
          <div class="basket-wrapper" @click="sendBasketEventToAnalitics">
            <nuxt-link to="/basket" tag="picture" :class="{'basket-active' : busketItems}"
                       :data-basketItems="busketItems">
              <img src="@/assets/images/svg/basket.svg" alt="" class="basket active">
            </nuxt-link>
          </div>
          <a href="tel:+380501909337" class="callback-phone" @click="sendDataToGoogleAnalitics">+38 (050) 190-93-37
            <span class="phone__description">Зателефонувати</span></a>
          <a href="tel:+380501909337" @click="sendDataToGoogleAnalitics">
            <img src="@/assets/images/svg/phone-icon.svg"
                 alt=""
                 class="navbar__mobile__callback">
          </a>
      ...
      <div class="nav__right-block-slide" :class="{activeXS: XSmenu, closeXs: !XSmenu}">
        ...
        <ul>
          ...
          <nuxt-link to="/contacts" tag="li" :class="{active: activeTab === 'Contacts'}">Контакти</nuxt-link>
        </ul>
      </div>
...
export default {
  ...
  computed: {
    ...
    busketItems() {
      return this.getBusketItems.length
    }
  },
  watch: {
    $route(to) {
