    ...
    <template v-else>
      <div class="basket-table">
        ...
        <table class="table table-head">
          <tr v-for="(item, i) in basketItems" :key="i">
            <td class="title">
              <nuxt-link :to="'/product/' + item.id" class="link">{{ item.title }}</nuxt-link>
            </td>
            <td class="quantity">
            ...
            <td class="delete-button">
              <svg @click="deleteItem(item.id)" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="trash"
                   class="delete-item svg-inline--fa fa-trash fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg"
                   viewBox="0 0 448 512">
                <path fill="#A8B6C7"
      ...
      <div class="buy-form">
        <div class="form-item input-item">
          <input class="name-input" :class="{ used: personalData.name.length }" type="text" v-model="personalData.name"
                 @blur="checkName">
          <label>ПІБ</label>
          <span class="error-handler name-error" v-if="nameError === true">Введіть ПІБ</span>
        </div>
        <div class="form-item input-item">
          <input :class="{ used: personalData.phone.length }" type="text" v-model="personalData.phone"
                 v-mask="'+38 (###) ###-##-##'" @blur="checkPhone">
          <label>Телефон</label>
        ...
        <div class="form-item input-item" v-click-outside="hideDropDawn">
          <input type="search" :class="{ used: currentCity.length }" v-model="currentCity" @focus="showDropdawn = true"
                 @blur="checkCity">
        ...
        <div class="select-wrapper">
          <select v-model="personalData.postOffice" class="selectPostOffice"
                  :disabled="NovaPoshtaPostOffices.length < 1"
                  :class="{'disabled-color': NovaPoshtaPostOffices.length < 1}">
...
import buyPopUp from "../components/basket/buyPopUp";
...
export default {
  ...
  computed: {
    ...mapGetters('basket', ['getBusketItems']),
    basketItems() {
      return this.getBusketItems
    },
    filteredCity() {
  ...
  methods: {
    ...mapMutations('basket', ['removeElFromBasket', 'minusQuantity', 'plusQuantity', 'cleanBasket']),
    goToGoods() {
    ...
    sendEmail(data) {
      return axios.post('http://7-sotok.com.ua/server/mail.php', JSON.stringify(data))
        .then(res => {
