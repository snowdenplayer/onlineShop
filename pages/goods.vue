  ...
  <section>
    <div class="goods-block container">
      <div class="goods-block__seeds-wrapper">
        ...
        >
          <nuxt-link :to="'/product/' + item.id" tag="picture">
            <img :src="item.img" alt="" class="product-title-img">
          </nuxt-link>

          <nuxt-link tag="p" :to="'/product/' + item.id" class="title">{{ item.title }}</nuxt-link>
          ...
          <div class="btn-price-wrapper">
            ...
            <div class="btns">
              <nuxt-link :to="'/product/' + item.id" tag="p" class="btn btn-more">Детальніше</nuxt-link>
              <p class="btn buy-btn" @click="buyAction(item)">Купити</p>
            </div>
          </div>

        </div>
        <div v-if="filteredShopItems.length === 0" class="sub-categories">Вибачте, товарів данної категорії немає у
          наявності<br>
          <nuxt-link to="/categories?mode=main_categories">усі категорії</nuxt-link>
        </div>
...
export default {
  ...
  computed: {
    ...mapGetters('itemsForShop', ['getShopItems']),
    shopItems() {
  ...
  methods: {
    ...
    buyAction(item) {
      ...
      this.addEllToBasket(busketItem)
